...
def get_youtube_video_id(url): # this title is scam lmao
    """
    Extracts the media ID from a URL supported by yt-dlp (audio/video).
    If the URL is invalid or not supported, returns None.
    """
    # Options to ensure we only extract metadata without downloading
    ydl_opts = {
        'quiet': True,
        'skip_download': True,
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # extract_info returns a dictionary with metadata, including "id"
...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress=None):
    ...
    vocals_path, instrumentals_path = run_mdx(mdx_model_params, song_output_dir, os.path.join('UVR-MDX-NET-Voc_FT.onnx'), orig_song_path, denoise=True, keep_orig=keep_orig)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        ai_vocals_path = os.path.join(song_dir, f'lead_{os.path.splitext(os.path.basename(orig_song_path))[0]}_{voice_model}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_{f0_method}{"" if f0_method != "mangio-crepe" else f"_{crepe_hop_length}"}.wav')
